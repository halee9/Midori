...
setInterval(function(){
    if (location.hash != hash)    {
    	...
    	/*
    	if (HLength >= history.length) {
	    	if (location.hash == "#checkout_container" ||
	    		location.hash == "#cart_container") {
		    		history.go(-1);
		    		return;
	    		}
    	}
    	...
    	*/
        set_order_total_button(location.hash);
        $(hash).removeClass("current").hide();
        hash = location.hash;
        if (location.hash == "#cart_container") {
	        create_cart_orders();
        }
        else if (location.hash == "#checkout_container") {
	        create_checkout();
        }
        else if (location.hash == "#order_complete") {
	        if (OrderSubmit == false) location.hash = "#menu_group_container";
	        else OrderSubmit = false;
        }
        $(location.hash).css("top","0px").addClass("current").show();
        /*
		$(location.hash).css("left","-100%").addClass("current");
...
function set_order_total_button(page){
    var order_total = recalc_cookie_orders_total().total;
    if (order_total > 0) {
    	$(page+' #header .right #cart_button').html("$"+order_total.toFixed(2)).css("visibility", "visible");
    }
    else {
    	$(page+' #header .right #cart_button').css("visibility", "hidden");
    }
}
...
function set_header_and_footer(container_id) {
	...
	set_order_total_button("#"+container_id);
    if (ss_username) {
    	$("#"+container_id+" #user_name").html(ss_username);
    	//$("#user_container").show();
    	$("#"+container_id+" #user_container").show();
...
function create_group_list(){
	...
	//var html = "<p style='padding: 0 10px'>TeriyakiOnline.com is closed from May 27th – May 30th due to floor renovation at Midori Teriyaki. <br>We will be back by June 2nd, Monday of next week. Sorry for any inconvenience and thank you very much for using TeriyakiOnline.com. <br>We will continue our best to serve fresh and delicious meal to you!</p>";
    var html = "<div class='group_big_container'>";

    for (var k in G_menugroup) {
    	var path = "../common/menupic/" + G_menugroup[k].pic;
		var photo = "<img src='../common/" + path + "' width='100px'>";
		var back_image = "background-image:url(\""+path+"\");background-size:cover;";
...
function create_menu_list(menugroup){
    ...
    set_header_and_footer($main.attr("id"));
    $main.find("#header .center").html(menugroup.name);
...
function create_menu_item(menu){
    ...
    set_header_and_footer("menu_item_container");
    $("#menu_item_container #header .center").html(menu.name);
    ...
    $('#menu_item_container #add_order').unbind(myUp);
...
function create_cart_orders(){
    ...
    */
    ...
    var html = "<div class='button_container'>"
        ...
        + "</div>";
    $('#cart_container').append(html);
    set_header_and_footer("cart_container");
    location.hash = "#cart_container";

    $('#checkout').bind(myUp, function(){
		if (TouchMove == true) return;
        if (ss_userid) {
        	location.hash = "#checkout_container";
        	//create_checkout();
        }
        else {
        	G_href = "checkout_container";
            location.hash = "#signin_container";
        }
    });
...
function create_checkout(){
    set_header_and_footer("checkout_container");
    $("#checkout_container #header .center").html("Checkout");
    ...
    if (get_order_total().amount <= 0) {
    	$("#place_order").attr("disabled","disabled").removeClass("able_effect").addClass("disable_effect");
    }
    else {
	    $("#place_order").attr("disabled",false).removeClass("disable_effect").addClass("able_effect");
    }
    ...
    */
    ...
    $('#place_order').unbind().bind(myUp, function(){
		if (TouchMove == true) return;
...
function create_signin(){
    set_header_and_footer("signin_container");
    $("#signin_container #header .center").html("Sign In");
    set_header_and_footer("signup_container");
    $("#signup_container #header .center").html("Sign Up");
    ...
    $('#go_signup').bind(myUp, function(){
		if (TouchMove == true) return;
    	location.hash = "#signup_container";
    });

}

function go_where(link) {
...
function order_complete_action(order_id) {
    ...
    set_header_and_footer("order_complete");
    $("#order_complete #header .center").html("Order Placed");
...
function get_selected_order(order_id) {
	location.hash = "#order_sheet_container";
    $.ajax({
        type: "POST",
        url: "../common/Orders.php",
...
function create_order_sheet(order){
    ...
    set_header_and_footer("order_sheet_container");
    $("#order_sheet_container #header .center").html("Order Details");
...
function create_my_page() {
	location.hash = "#my_page_container";
    set_header_and_footer("my_page_container");
    $("#my_page_container #header .center").html("My Page");
...
function create_store_info(store){
    ...
    set_header_and_footer("store_info_container");
    $("#store_info_container #header .center").html("Store Information");
...
function create_button_event(){
    ...
    $('#cart_button').bind(myUp, function() {
		if (TouchMove == true) return;
		location.hash = "#cart_container";
		//create_cart_orders();
    });
    ...
    $('#info_button').bind(myUp, function() {
		if (TouchMove == true) return;
        location.hash = "#store_info_container";
    });
