...
function create_order_processing_header(){
    var html
		= "<div style='background-color:#333;color:#ddd;text-align:right;padding:10px;'>"
		...
		+ "<div class='cart_label' style='clear:both'>"
			+ "<span style='float:right; width:110px; text-align:center; padding-right:10px;'>Customer</span>"
    ...
    for (var i in order){
        ...
        for (var j in order[i].items){
			...
			for (var k in order[i].items[j].options) {
				...
				option_array[array_cnt] = abbr_name;
				array_cnt++;
			}
		...
		center_html += "</table>";
		var html = "<div class='order_processing_list' style='border-bottom: 1px solid #aaa'>"
			...
			+ "</div>"
			+ "</div>"
			+ "<div style='text-align:right; background-color:#eee;'>"
...
function init() {
    $(document).ready(function() {
        create_header(G_store.name, "Order Control");
        var html = "<div id='list_head'></div><div id='order_list'></div>";
        $('#main_container').html(html);
        
        create_order_processing_header();
